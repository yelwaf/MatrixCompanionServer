...
public class PodioDate implements Serializable {
	...
	public FieldValuesUpdate getFieldValuesUpdate(String externalId) {
		if (!isStartDate() && isEndDate()) {
			throw new IllegalStateException(
					"Must have a start date, when having an end date!");
		}
		HashMap<String, String> dateHashMap = new HashMap<String, String>();
		if (isStartDate()) {
			dateHashMap
					.put("start", isStartTime() ? PODIO_DATE_TIME_FORMATTER
							.get().format(start) : (PODIO_DATE_FORMATTER.get()
							.format(start) + " 00:00:00"));
			dateHashMap.put("start_date",
					PODIO_DATE_FORMATTER.get().format(start));
			dateHashMap.put("start_time", isStartTime() ? PODIO_TIME_FORMATTER
					.get().format(start) : null);
		}
		if (isEndDate()) {
			dateHashMap.put("end", isEndTime() ? PODIO_DATE_TIME_FORMATTER
					.get().format(end) : (PODIO_DATE_FORMATTER.get()
					.format(end) + " 00:00:00"));
			dateHashMap.put("end_date", PODIO_DATE_FORMATTER.get().format(end));
			dateHashMap.put("end_time", isEndTime() ? PODIO_TIME_FORMATTER
					.get().format(end) : null);
		}
	...
	public String toString() {
		return "PodioDate [start=" + start + ", startTime=" + startTime
				+ ", startDate=" + startDate + ", end=" + end + ", endTime="
